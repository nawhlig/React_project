...
export default function Todo_list_form({visible, setVisible, todolists, setTodolists}) {
  //페이지 리로딩 상태변화
          ...
          .then(response=>{
              const fetchTodolists = async () => {
                    try {// 요청이 시작 할 때에는 error 와 Todolist 를 초기화하고
                        ...
                        await setTodolists(prev => ({...prev, inprogress:response.data}));

                        const response3 = await API.get("todo/?status=종료");  
                        await setTodolists(prev => ({...prev, end:response.data}));
                        // form.resetFields()
                      }
